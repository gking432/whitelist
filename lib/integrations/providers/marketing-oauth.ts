...
async function googleToken(client: MarketingOAuthClient, params: Record<string, string>) { const response = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: client.clientId, client_secret: client.clientSecret, ...params }), signal: AbortSignal.timeout(15_000) }); if (!response.ok) throw new Error(`Google authorization failed (${response.status}).`); return response.json() as Promise<{ access_token?: string; refresh_token?: string; expires_in?: number }>; }
...
export async function exchangeGoogleMarketingCode(providerKey: "google_ads" | "google_business_profile", client: MarketingOAuthClient, code: string, developerToken?: string): Promise<GoogleAdsCredentials | GoogleBusinessProfileCredentials> {
  ...
  if (providerKey === "google_ads") {
    ...
    if (!response.ok) throw new Error(`Google Ads account discovery failed (${response.status}).`);
  ...
  if (!accountsResponse.ok) throw new Error(`Business Profile account discovery failed (${accountsResponse.status}).`);
  ...
  if (!locationsResponse.ok) throw new Error(`Business Profile location discovery failed (${locationsResponse.status}).`);
...
export async function exchangeMetaCode(client: MarketingOAuthClient, code: string): Promise<MetaCredentials> {
  ...
  if (!tokenResponse.ok) throw new Error(`Meta authorization failed (${tokenResponse.status}).`);
  ...
  if (!pagesResponse.ok) throw new Error(`Meta Page discovery failed (${pagesResponse.status}).`);
...
async function podiumToken(client: MarketingOAuthClient, body: Record<string, string>) { const response = await fetch("https://api.podium.com/oauth/token", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ client_id: client.clientId, client_secret: client.clientSecret, ...body }), signal: AbortSignal.timeout(15_000) }); if (!response.ok) throw new Error(`Podium authorization failed (${response.status}).`); return response.json() as Promise<{ access_token?: string; refresh_token?: string; expires_in?: number }>; }
