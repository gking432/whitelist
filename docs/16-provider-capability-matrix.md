...
| Managed Twilio           | Partner parent-account validation; client subaccount/number provisioning; signed inbound SMS and voice; approval-gated SMS; full-duplex OpenAI Realtime answering with barge-in and Gather failover; staff forwarding; Media Streams transcription; caller matching; post-call CRM/workflow actions | AI answering and the live staff coach require the always-on voice-stream service; staff popups also require the desktop app |
