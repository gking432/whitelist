...
| Twilio outbound AI callbacks | Real contract and locally proven runtime — authorized client staff can start lead follow-up, reschedule, or reminder calls from the CRM; the app dials through the client's live Twilio subaccount, matches the customer before connection, streams the same tenant agent, handles completed/busy/no-answer outcomes, and ends the exact carrier call after final AI audio. Spoof sessions cannot dial |
...
| Production health and abuse controls | Real — database and latest-schema-aware `/api/health`, durable atomic Postgres rate limits with bounded outage fallback, security headers, and Render health routing |
...
| Automated multichannel release journey | Real — the built production server is exercised through form/webhook, forwarded-email envelope, signed Twilio SMS, public website chat, authenticated AI phone scheduling/booking, and desktop assistant HTTP surfaces; resulting CRM, approvals, events, permissions, support routing, connector-worker failure/backoff/retry/stale-lease recovery, and cleanup are asserted against a fresh database in CI |
