...
| Automated multichannel release journey | Real — the built production server is exercised through form/webhook, forwarded-email envelope, signed Twilio SMS, public website chat, authenticated AI phone scheduling/booking, and desktop assistant HTTP surfaces; resulting CRM, approvals, events, permissions, support routing, and cleanup are asserted against a fresh database in CI |
